...
public abstract class AbstractCircuitBreaker implements CircuitBreaker {
    ...
    protected void updateNextRetryTimestamp() {
        this.nextRetryTimestamp = TimeUtil.currentTimeMillis() + retryTimeoutMs;
    }
    ...
    protected boolean fromCloseToOpen(double snapshotValue) {
        ...
        if (currentState.compareAndSet(prev, State.OPEN)) {
            ...
            for (CircuitBreakerStateChangeObserver observer : observerRegistry.getStateChangeObservers()) {
                observer.onTransformToOpen(prev, rule, snapshotValue);
            }
            return true;
        }
        return false;
    ...
    protected boolean fromOpenToHalfOpen() {
        if (currentState.compareAndSet(State.OPEN, State.HALF_OPEN)) {
            for (CircuitBreakerStateChangeObserver observer : observerRegistry.getStateChangeObservers()) {
                observer.onTransformToHalfOpen(State.OPEN, rule);
            }
            return true;
        }
        return false;
    ...
    protected boolean fromHalfOpenToOpen(double snapshotValue) {
        if (currentState.compareAndSet(State.HALF_OPEN, State.OPEN)) {
            updateNextRetryTimestamp();
            for (CircuitBreakerStateChangeObserver observer : observerRegistry.getStateChangeObservers()) {
                observer.onTransformToOpen(State.HALF_OPEN, rule, snapshotValue);
            }
            return true;
        }
        return false;
    ...
    protected boolean fromHalfOpenToClose() {
        if (currentState.compareAndSet(State.HALF_OPEN, State.CLOSED)) {
            resetStat();
            for (CircuitBreakerStateChangeObserver observer : observerRegistry.getStateChangeObservers()) {
                observer.onTransformToClosed(State.HALF_OPEN, rule);
            }
            return true;
        }
        return false;
